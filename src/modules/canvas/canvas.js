(function() {
  ...
  function init() {
    for( var i = 0; i < numberOfLayers; i++ ) {
      layers.push({
        x: width / 2 + Math.sin( i / numberOfLayers * 2 * Math.PI ) * ( radius - layerSize ),
        y: height / 2 + Math.cos( i / numberOfLayers * 2 * Math.PI ) * ( radius - layerSize ),
  ...
  function paint() {
    ...
    for( let i = layersLength - layerOverlap, len = layersLength; i < len; i++ ) {
      context.save();
